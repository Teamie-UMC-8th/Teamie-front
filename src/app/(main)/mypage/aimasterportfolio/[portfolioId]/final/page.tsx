...
export default function AIMasterPortfolioPage() {
  ...
  // 프로젝트 정보 가져오기
  const { data: projectHomeData, isLoading: projectLoading } = useProjectHome(
    portfolioDetail?.projectId || 0
  );
  ...
  const handleCategoryChange = (newCategory: CategoryKey) => {
    ...
    if (portfolioDetail) {
      patchMasterPortfolio.mutate({
        portfolioId,
        body: {
          detailInfo: portfolioDetail.detailInfo || '',
          assignedTask: portfolioDetail.assignedTask || '',
          keyAchievement: portfolioDetail.keyAchievement || '',
          insight: portfolioDetail.insight || '',
          contributionRate: contribution,
          mainTask: portfolioDetail.mainTask || '',
