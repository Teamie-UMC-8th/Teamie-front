...
export default function ReplyComment({
  ...
  if (replyToIndex === idx) {
    return (
      <div className="flex items-center ml-[20px] w-[1340px]">
        <img
            ...
            {editCocommentId && editCocommentId === cocommentIdx ? (
              <div className="flex items-center">
                <input
                  ...
                  className="rounded-[8px] w-[1224px] min-h-[46px] pl-[12px] py-[10px] bg-white border-[2px] border-[#BBBBBB]
                  max-lg:w-[671px] mr-[8px]"
                  placeholder="대댓글을 수정하세요"
                />
                <button
                  onClick={onCocommentEditSubmit}
                  disabled={!editCocommentContent.trim()}
                  className="w-[36px] h-[36px] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
              ...
              max-lg:w-[671px] group"
              >
                {submittedReply}
              </div>
            )}
            {/* 대댓글 드롭다운 메뉴 */}
            {!editCocommentId && onCocommentEdit && onCocommentDelete && (
              <CommentMenuDropdown
                type="cocomment"
                onSelect={(action) => {
                  if (action === 'edit') {
